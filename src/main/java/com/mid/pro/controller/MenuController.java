...
public class MenuController {
	...
	public ModelAndView menuList(MenuVO menuVO, RestaurantVO restaurantVO, MenuFilesVO menuFilesVO) throws Exception{
		ModelAndView mv = new ModelAndView();
		List<MenuVO> list = menuService.menuList(menuVO);
		restaurantVO = restaurantService.restSelect(restaurantVO);
		menuFilesVO = menuService.fileSelect2(menuFilesVO);
		...
		if (menuVO != null) {
			...
			mv.addObject("msg", "메뉴가 존재하지 않습니다..");
	...
	public ModelAndView menuWrite(MenuListVO menuListVO, HttpSession session) throws Exception{
		...
		String msg = "메뉴 입력에 실패하였습니다.";

		if (result > 0) { mv.setViewName("redirect:../restaurant/restList"); 
		} else {
