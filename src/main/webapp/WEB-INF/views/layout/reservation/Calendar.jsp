...
<div class="section">
	...
	<div class="container">
		<form class="form-horizontal" role="form" id="goreservation" name="goreservation"  method="POST">
			...
			<div class="row">
				<h2>예 약</h2>
				<h5>(버튼은 예약가능 시작 시간입니다. 1시간 단위로 테이블을 선택해 예약하실 수 있습니다. ex) 10:00
					=> 10:00~11:00)</h5>
				<br>
				<br>
				<h4>예약 날짜</h4>
				<div class="form-group">
					<div class="col-md-3 text-left">
						<input type="date" class="form-control" id="rev_date"
							name="rev_date" value="${frev_date}" readonly>
					</div>
					<div class="col-md-4 text-left">
						<a class="btn btn-default" href="../reservation/reservationWrite?rest_num=${backupnum}">되돌아가기</a>
					</div>
			...
			<div class="row">
				...
				<c:forEach items="${table}" var="taable" varStatus="i">
					<div class="col-md-12">
						...
						<%-- //${ta.tablenum} --%>
						<br>
						<%-- height="100" width="100"  src="<%=adminctx %>/restaurant/table/${ta.tableuser }.jsp" --%>
						<!-- 시간을 보여주는 for -->
						...
						<c:forEach var="t" items="${tarr}" varStatus="k">
							...
							<c:if test="${table.table_num eq t.table_num }">
								...
								<c:if test="${t.rev_min==0 }">
									...
									<a id="a${k.index}" class="btn btn-default"
			...
			<input type="hidden" name="rest_num" id="rest_num" value="${backupnum }">
...
var reservation=function(vtime,vtable ){
	...
	if(yn){
		...
		goreservation.action="../reservation/reservationWrite";
